...
def test(cfg: DictConfig):
    ...
    cfg.data.split = cfg.test_category
    cfg.data.fast_evaluation = False
    val_dataloader = DataLoader(
        instantiate(cfg.data),
        batch_size=int(cfg.machine.batch_size // 4),
        num_workers=cfg.machine.num_workers,
        shuffle=False,
    )
    logging.info(f"Testing {cfg.test_category} ...")
    model.testing_category = cfg.test_category
    trainer.test(
        model,
        dataloaders=val_dataloader,
        ckpt_path=(cfg.model.checkpoint_path),
    )
    logging.info(f"Testing {cfg.test_category} done")
